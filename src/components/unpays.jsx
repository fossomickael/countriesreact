...
class UnPays extends Component {
  ...
  findCountry = (code) => {
    const country = this.props.pays.find(
      (unpays) => unpays.alpha3Code === code
    );
    if (country) {
      return country.name;
    }
        ...
        <div className="unpays">
          ...
          <div className="paysdroite">
            ...
            <div className="unpaysdetails">
              <ul>
                <li>
                  <b>Native Name:</b> {`${this.props.unpays.nativeName}`}
                </li>
                <li>
                  <b>Population:</b> {`${this.props.unpays.population}`}
                </li>
                <li>
                  <b>Region:</b> {`${this.props.unpays.region}`}
                </li>
                <li>
                  <b>Capital:</b> {`${this.props.unpays.capital}`}
                </li>
              </ul>
              <ul className="otherdetails">
                <li>
                  <b>Top level domain:</b>{" "}
                  {`${this.props.unpays.topLevelDomain}`}
                </li>
                <li>
                  <b>Currencies:</b>{" "}
                  {`${this.extractvalues(
                    this.props.unpays.currencies,
                    "name"
                  )}`}
                </li>
                <li>
                  <b>Languages:</b>{" "}
                  {`${this.extractvalues(this.props.unpays.languages, "name")}`}
                </li>
              </ul>
            </div>
            <div className="border">
              <b>Border Countries:</b>{" "}
              {`${this.extractborders(this.props.unpays.borders)}`}
            </div>
          </div>
        </div>
      </div>
